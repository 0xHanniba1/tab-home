/* ================================================================
   tab-home — Chrome Tabs State and Actions

   Thin wrapper around chrome.tabs/chrome.windows plus the in-memory
   grouped-tab state used by the renderer and event handlers.
   ================================================================ */
...
/* ----------------------------------------------------------------
   DISPLAY TITLE OVERRIDES — local to tab-home
   ---------------------------------------------------------------- */

function sanitizeTabTitleOverrides(raw) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return {};
  const clean = {};
  for (const [url, title] of Object.entries(raw)) {
    const cleanUrl   = String(url || '').trim();
    const cleanTitle = typeof title === 'string' ? title.trim() : '';
    if (cleanUrl && cleanTitle) clean[cleanUrl] = cleanTitle;
  }
  return clean;
}

async function getTabTitleOverrides() {
  try {
    const { [TAB_TITLE_OVERRIDES_KEY]: raw = {} } =
      await chrome.storage.local.get(TAB_TITLE_OVERRIDES_KEY);
    return sanitizeTabTitleOverrides(raw);
  } catch {
    return {};
  }
}

async function setTabTitleOverride(url, title) {
  const cleanUrl = String(url || '').trim();
  if (!cleanUrl) return;

  const overrides = await getTabTitleOverrides();
  const cleanTitle = String(title || '').trim();
  if (cleanTitle) overrides[cleanUrl] = cleanTitle;
